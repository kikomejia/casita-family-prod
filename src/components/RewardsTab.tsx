...
export function RewardsTab() {
  ...
  useEffect(() => {
    ...
    const unsubLogs = base44.entities.RewardLog.subscribe((logs) => {
      setRewardLogs(logs);
    }, "-claimed_at", 10);

    const unsubSettings = base44.entities.Settings.subscribeOne("family", (s) => {
      if (s && s.prizes) setPrizes(s.prizes);
    });
  ...
  const handleSpinDone = async (prize: string) => {
    ...
    <div className="p-4 space-y-6">
      {!spinningMember ? (
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold overflow-hidden shrink-0" style={{ backgroundColor: member?.color || '#ccc' }}>
                          {member?.avatar_url ? (
                            ...
                            member?.name?.charAt(0) || log.member_name?.charAt(0) || '?'
                          )}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-gray-800">{log.member_name} won {log.prize}</p>
        ...
        <div className="flex flex-col items-center justify-center py-8 space-y-6">
          ...
          <button
            onClick={() => setSpinningMember(null)}
